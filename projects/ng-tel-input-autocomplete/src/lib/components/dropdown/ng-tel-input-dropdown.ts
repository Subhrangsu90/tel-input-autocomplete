...
export class NgTelInputDropdown implements OnChanges, AfterViewInit {
  ...
  getHighlightedText(text: string, search: string): SafeHtml {
    ...
    const highlighted = escapedText.replace(new RegExp(`(${escapedSearch})`, 'gi'), '<mark style="background:var(--ngti-color-highlight-bg,#dbeafe);color:var(--ngti-color-highlight-text,#172554);font-weight:700;border-radius:.2rem;padding:0 .05rem">$1</mark>');
